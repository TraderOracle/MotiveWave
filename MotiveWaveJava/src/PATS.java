...
public class PATS extends Study
{
  ...
  public void initialize(Defaults defaults)
  {
    ...
    grp.addRow(new BooleanDescriptor("SHOWUP", "Show Long Entries", true));
    grp.addRow(new BooleanDescriptor("SHOWDOWN", "Show Short Entries", true));

    var grp1 = tab.addGroup("Markers");
    grp1.addRow(new MarkerDescriptor("UP", "Up Marker", Enums.MarkerType.CIRCLE,
        Enums.Size.SMALL, GREEN, RED, true, true));
    grp1.addRow(new MarkerDescriptor("DOWN", "Down Marker", Enums.MarkerType.CIRCLE,
        Enums.Size.SMALL, RED, RED, true, true));

    RuntimeDescriptor desc = new RuntimeDescriptor();
    setRuntimeDescriptor(desc);
    desc.declareSignal(Signals.UP, "Long Signal");
    desc.declareSignal(Signals.DOWN, "Short Signal");
  }
  ...
  private int FindLastGreen(DataSeries s, int index, double ema)
  {
    try {
      for (int i = index - 1; i > 0; i--){
        Color ch = GetCandleColor(s, i);
        double ema21 = s.ema(i, 21, Enums.BarInput.CLOSE);
        if (ch == GREEN && s.getClose(index) < s.getOpen(i) && ema < ema21)
          return i;
      }
    }
    catch (Exception e) {}
  ...
  protected void calculate(int index, DataContext ctx)
  {
    ...
    if (cc == GREEN) {
      ...
      int ix = FindLastGreen(s, index, ema21);
      debug("ix " + ix + " index " + index);
      if (ix > 0) {
        var marker = getSettings().getMarker("DOWN");
        Coordinate coords = new Coordinate(s.getStartTime(ix), (double) s.getHigh(ix)+0.6);
