...
public class TONews extends Study
{
    ...
    {
        debug("FetchTheNews");
        String htmlSource = "<div class=\"column column--primary\"> <div class=\"element element--tableblock\"> "
            + "<table><thead><tr><th style=\"text-align: left;\">Time (ET)</th><th style=\"text-align: left;\">Report</th><th style=\"text-align: left;\">Period</th><th style=\"text-align: left;\">Actual</th><th style=\"text-align: left;\">Median Forecast</th><th style=\"text-align: left;\">Previous</th></tr></thead>"
            + "<tbody><tr><td style=\"text-align: left;\"><b>MONDAY, MARCH 31</b></td><td style=\"text-align: left;\"></td><td style=\"text-align: left;\"></td><td style=\"text-align: left;\"></td><td style=\"text-align: left;\"></td><td style=\"text-align: left;\"></td></tr>"
            + "<tr><td style=\"text-align: left;\">9:45 am</td><td style=\"text-align: left;\">Chicago Business Barometer (PMI)</td><td style=\"text-align: left;\">March</td><td style=\"text-align: left;\"></td><td style=\"text-align: left;\">43.6</td><td style=\"text-align: left;\">45.5</td></tr></tbody></table></div></div>";

        ArrayList<Map<String, String>> calendarEntries = new ArrayList<>();

        try {
            Document doc = Jsoup.parse(htmlSource);
            Element table = doc.select("div.column.column--primary div.element.element--tableblock table").first();

            if (table == null) {
                System.err.println("Could not find the calendar table in the HTML source.");
            ...
            for (Element row : rows) {
                ...
                if (!cells.isEmpty() && cells.get(0).select("b").size() > 0) {
                    currentDate = cells.get(0).text().trim();
                    continue;
                }
                ...
                if (cells.size() >= 6) {
                    ...
                    entry.put("time", cells.get(0).text().trim());
                    entry.put("report", cells.get(1).text().trim());
                    entry.put("period", cells.get(2).text().trim());
                    entry.put("actual", cells.get(3).text().trim());
                    entry.put("forecast", cells.get(4).text().trim());
                    entry.put("previous", cells.get(5).text().trim());
            ...
            System.err.println("Error parsing economic calendar: " + e.getMessage());
            e.printStackTrace();
        }

        debug("cal entries: " + calendarEntries.size());
